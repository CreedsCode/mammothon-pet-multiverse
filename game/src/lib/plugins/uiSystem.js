...
let uiDefaultButtonColor = hsl(0, 0, 0.5);
let uiDefaultHoverColor = hsl(0, 0, 0.7);
...
function initUISystem(context = overlayContext) {
  ...
  function uiUpdate() {
    function updateObject(o) {
      ...
      o.update();
      for (const c of o.children) updateObject(c);
    }
...
function drawUILine(
  posA,
  posB,
  thickness = uiDefaultLineWidth,
  color = uiDefaultLineColor
) {
...
class UIObject {
  ...
  update() {
    ...
    if (!mouseDown || isTouchDevice) {
      ...
      if (!mouseDown && isTouchDevice) this.mouseIsOver = false;
      if (this.mouseIsOver && !mouseWasOver) this.onEnter();
      if (!this.mouseIsOver && mouseWasOver) this.onLeave();
    }
    if (mouseWasPressed(0) && this.mouseIsOver) {
      this.mouseIsHeld = true;
      this.onPress();
      if (isTouchDevice) this.mouseIsOver = false;
...
class UIButton extends UIObject {
  ...
  render() {
    ...
    const textSize = vec2(this.size.x, this.size.y * 0.8);
    drawUIText(
      this.text,
      this.pos,
      textSize,
...
class UICheckbox extends UIObject {
  ...
  render() {
    ...
    if (this.checked) {
      ...
      drawUILine(
        this.pos.add(this.size.multiply(vec2(-0.5, -0.5))),
        this.pos.add(this.size.multiply(vec2(0.5, 0.5))),
        this.lineWidth,
        this.lineColor
      );
...
class UIScrollbar extends UIObject {
  constructor(pos, size, value = 0.5, text = "") {
    super(pos, size);
    this.value = value;
    this.text = text;
    this.color = uiDefaultButtonColor;
